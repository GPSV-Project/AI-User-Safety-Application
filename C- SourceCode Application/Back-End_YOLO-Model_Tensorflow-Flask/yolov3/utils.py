...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    print ("This is the value of bboxes: ", bboxes)
    # empty = []
    if bboxes:
        max_element = 0
        max_index = 0
        for i, array in enumerate (bboxes):
            print (array)
            # max_element = max (max_element, array[i][4])
            if array[4] > max_element:
                max_element = array[4]
                max_index = i
        ...
        score = max_element
        class_ind = int(bboxes[max_index][5])
        NUM_CLASS = read_class_names(YOLO_COCO_CLASSES)
        class_ind = NUM_CLASS[class_ind]

    else:
        null_score = ""
        null_class_ind = ""
        return image, null_score, null_class_ind

    # score = bboxes[0][4]
    # class_ind = int(bboxes[0][5])
    # NUM_CLASS = read_class_names(YOLO_COCO_CLASSES)
    # class_ind = NUM_CLASS[class_ind]
    
    print (f"The confidence score is {score} and class detection is {class_ind}")

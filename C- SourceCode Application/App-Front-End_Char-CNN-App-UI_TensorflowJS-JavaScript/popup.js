...
const yoloDomains = ['www.wellsfargo.com', 'www.bankofamerica.com', 'www.facebook.com', 'www.amazon.com', 'www.google.com']
...
getCurrURL(queryInfo).then(function(result) {
  ...
  getPhishTankResults(activeURL).then(function(result) {
    ...
    console.log('phishtank json= ' + JSON.stringify(jsonObj));
      ...
      charCnn.runCharCnn(trancatedURL).then(function(result) {
        ...
        getScreenshotDataURI().then(function(result) {
          ...
          yolo.runYolo(dataURI).then(function(result) {
            ...
            else if (confScore >= 0.7 && charCNNResult === "Legitimate") {
              const domain = url_domain(activeURL);
              if (yoloDomains.indexOf(domain) < 0) {
            ...
            else if (charCNNResult === "Phishing") {
              showMainResults("Suspicious");
            }
            else {
              showMainResults("Legitimate");
